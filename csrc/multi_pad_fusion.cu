...
__global__ void __launch_bounds__(kBlockSize) multi_pad_transpose_kernel(MultiPadBaseArgs args) {
  ...
  constexpr size_t bdimy = kWarpsPerTile;
  ...
  // load/store
  constexpr size_t num_iterations = THREADS_PER_WARP / kWarpsPerTile;
...
__global__ void __launch_bounds__(kNumThreadsPerBlock)
    multi_pad_cast_transpose_kernel(MultiPadCastTransposeArgs args) {
...
void MultiPadTranspose(std::vector<at::Tensor> input_list, std::vector<at::Tensor> output_list) {
  if (input_list.empty()) {
    return;
  }
  ...
  for (size_t tensor_id = 0; tensor_id < input_list.size(); ++tensor_id) {
    TORCH_CHECK(input_list[tensor_id].scalar_type() == output_list[tensor_id].scalar_type());
