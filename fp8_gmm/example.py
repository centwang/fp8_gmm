...
def topk(scores, top_k, moe_renorm=False):
    ...
    weights = torch.nn.functional.softmax(scores, dim=1, dtype=torch.float)
...
class Fp8MlpModule(torch.nn.Module):
    def __init__(self, n_embd, n_inner, n_experts, top_k, dtype=torch.bfloat16):
        super(Fp8MlpModule, self).__init__()
        self.n_embd = n_embd
        self.n_inner = n_inner
        self.n_experts = n_experts
        self.top_k = top_k
        self.sort_end_bit = max(int(np.ceil(np.log2(n_experts))), 1)
        self.gating_network = torch.nn.Linear(n_embd, n_experts, bias=False).to(dtype).to("cuda")
        self.grouped_mlp = GroupedMlp(n_embd, n_inner, n_experts, dtype=dtype)

    def compute(self, x, tokens_per_expert, indices, bin_ids, expert_weights, bins, top_k):
        x = x.view(-1, x.size(-1))
        x = mega_ops.gather(x, indices, bin_ids, bins, top_k)
        batch_sizes = tokens_per_expert.cpu().to(torch.long)
        out = self.grouped_mlp(x, batch_sizes)
        return mega_ops.scatter(out, indices, bin_ids, expert_weights, bins, top_k)
...
# Fp8LinearModule, using cublasLtMatmul.
model_fp8_linear_cublas = Fp8LinearModule(hidden_states, num_inner, num_experts, top_k, dtype=dtype, cutlass=False)
fp8_recipe = recipe.DelayedScaling(margin=0, interval=1, fp8_format=recipe.Format.HYBRID)
for _ in range(warmup_steps):
    input = torch.randn(num_tokens, hidden_states, dtype=dtype, device="cuda", requires_grad=True)
    with tex.fp8_autocast(enabled=True, fp8_recipe=fp8_recipe):
        out = model_fp8_linear_cublas(input)
    out.backward(torch.randn(*out.size(), dtype=dtype, device="cuda"))
torch.cuda.synchronize()
start = time()
for _ in range(run_steps):
    ...
    out.backward(torch.randn(*out.size(), dtype=dtype, device="cuda"))
torch.cuda.synchronize()
print(f"Fp8LinearModule cublasLtMatmul run: {(time() - start) * 1000 / run_steps:.3f} ms")

# Fp8LinearModule, using cutlass grouped gemm.
model_fp8_linear_cutlass = Fp8LinearModule(hidden_states, num_inner, num_experts, top_k, dtype=dtype, cutlass=True)
for _ in range(warmup_steps):
    input = torch.randn(num_tokens, hidden_states, dtype=dtype, device="cuda", requires_grad=True)
    with tex.fp8_autocast(enabled=True, fp8_recipe=fp8_recipe):
        out = model_fp8_linear_cutlass(input)
    out.backward(torch.randn(*out.size(), dtype=dtype, device="cuda"))
torch.cuda.synchronize()
start = time()
for _ in range(run_steps):
    ...
    out.backward(torch.randn(*out.size(), dtype=dtype, device="cuda"))
torch.cuda.synchronize()
print(f"Fp8LinearModule cutlass run: {(time() - start) * 1000 / run_steps:.3f} ms")

# Fp8MlpModule
model_fp8_mlp = Fp8MlpModule(hidden_states, num_inner, num_experts, top_k, dtype=dtype)
for _ in range(warmup_steps):
    input = torch.randn(num_tokens, hidden_states, dtype=dtype, device="cuda", requires_grad=True)
    with tex.fp8_autocast(enabled=True, fp8_recipe=fp8_recipe):
        out = model_fp8_mlp(input)
    out.backward(torch.randn(*out.size(), dtype=dtype, device="cuda"))
torch.cuda.synchronize()
start = time()
for _ in range(run_steps):
    ...
    out.backward(torch.randn(*out.size(), dtype=dtype, device="cuda"))
torch.cuda.synchronize()
print(f"Fp8MlpModule run: {(time() - start) * 1000 / run_steps:.3f} ms")
